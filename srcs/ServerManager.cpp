...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/29 09:19:31 by spapyan           #+#    #+#             */
/*   Updated: 2023/08/29 09:19:31 by spapyan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ServerManager::runServers() {
    ...
    while (true) {
        ...
        if ((select_ret = select(_biggest_fd + 1, &recv_set_cpy, &write_set_cpy, NULL, &timer)) < 0) {
            Logger::logMsg(RED, CONSOLE_OUTPUT, "webserv: select error %s   Closong....", strerror(errno));
            exit(1);
            continue;
        }
...
void    ServerManager::checkTimeout() {
    for (set::map<int, Client>::iterator it = _clients_map.begin(); it != _clients_map.end(); ++it) {
...
void    ServerManager::acceptNewConnection(ServerConfig& serv) {
    ...
    if ((client_sock = accept(serv.getFd(), (struct sockaddr *)&client_address, 
        (socket_t*)&client_address_size)) == -1) {
...
void    ServerConfig::closeConnection(const int i) {
...
void    ServerManager::sendResponse(const int& i, Client& c) {
    ...
    if (bytes_sent < 0) {
        ...
        if (c.request.keepAlive() == false || c.response.errorCode() || c.response.getCgiState()) {
...
void    ServerManager::sendCgiBody(Client& c, CgiHandler& cgi) {
    ...
    if (req_body.length() == 0)
        ...
        bytes_sent = write(cgi.pipr_in[1], req_body.c_str(), req_body.length());
